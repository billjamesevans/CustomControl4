...
class Light:
    ...
    async def get_state(self):
        ...
        return item_info.get("properties", {}).get("state", None)

    async def set_level(self, level: int, ramp_rate: int = 0):
        if not 0 <= level <= 100:
            raise ValueError("Level must be between 0 and 100.")
        command = "RAMP_TO_LEVEL"
        params = {"level": level, "ramp_rate": ramp_rate}
        await self.director.send_command(self.item_id, command, params)
